...
function Podcast({ checkit }) {
  ...
  const delete_this_shit = async (pk) => {
    await AXIOS.delete(`podcast/${pk}/delete`)
      .then((res) => {
        ...
        toast("Podcast sters cu succes!");
      })
      .catch((err) => {
        console.log(err);
        toast("Podcast nu a fost sters!");
      });
  };
  ...
  const uploadYoga = async () => {
    ...
    await AXIOS.post("podcast/create", form, {
      headers: {
      ...
      .then((res) => {
        ...
        toast("Podcast adaugata cu succes!");
      })
      .catch((err) => {
        console.log(err);
        toast("Poga nu a fost adaugata!");
      });
  ...
  useEffect(() => {
    const get = async () => {
      await AXIOS.get("podcast/").then((res) => {
              ...
              data.map((da, index) => {
                ...
                return (
                  <tr key={da[0]}>
                    ...
                    <td>{da[1].plays} </td>
                    <td>
                      {/* {new Boolean(da[1].premium).toString()} */}
                      {da[1].premium ? (
